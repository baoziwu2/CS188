...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(4):
        if not state[1][i]:
            res = max(res, manhattan_distance(state[0], corners[i]))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foodGrid.asList(): return 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        while not queue.isEmpty():
            current_position, actions = queue.pop()
            if problem.isGoalState(current_position):
                return actions
            if current_position not in visited:
                visited.add(current_position)
                for successor, action, stepCost in problem.getSuccessors(current_position):
